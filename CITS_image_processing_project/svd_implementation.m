...
% Principal components analysis (singular value decomposition) correction
[U,S,V] = svd(data,'econ');

PC_cutoff = 11;
denoised_data = U(:,1:PC_cutoff) * S(1:PC_cutoff,1:PC_cutoff) * V(:,1:PC_cutoff)';

% Shape data into stack of images for further denoising
denoised_data_mat3 = reshape(denoised_data,row_dimension,column_dimension,page_dimension);

% Correct the axes if necessary
denoised_data_mat3 = permute(denoised_data_mat3,[2 1 3]);

%% Filter the data in multiple dimensions
...
for page_counter = 1:page_dimension
    layerfiltered_data_mat3(:,:,page_counter) = filter2(h_surface,denoised_data_mat3(:,:,page_counter));
end
...
figure(1)
clf
imagesc(denoised_data_mat3(:,:,page))

figure(2)
clf
plot(permute(denoised_data_mat3(row,column,:),[3 2 1]))
...
clf
hold on
plot(permute(denoised_data_mat3(row,column,:),[3 2 1]),'Color','r','LineStyle','--')
