...
window_size = 7;   % size of window for 1-D current data filtering (must be odd)
...
% 2-D filtering image layers with a constructed parabolic filter
...
% h_perpendicular = [1/15 1/15 1/15 1/15 1/15 1/15 1/15];
...
for row_counter = 1:row_dimension
    for column_counter = 1:column_dimension
        smoothed_data_mat3(row_counter,column_counter,:) = filtfilt(h_perpendicular,1,layerfiltered_data_mat3(row_counter,column_counter,:));
    end
end
